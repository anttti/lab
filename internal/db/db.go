...
`
	...
	// Add draft column to merge_requests if it doesn't exist.
	err = db.QueryRow(`SELECT COUNT(*) FROM pragma_table_info('merge_requests') WHERE name = 'draft'`).Scan(&count)
	if err != nil {
		return err
	}
	if count == 0 {
		if _, err := db.Exec(`ALTER TABLE merge_requests ADD COLUMN draft BOOLEAN NOT NULL DEFAULT 0`); err != nil {
			return err
		}
	}
