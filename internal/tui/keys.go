...
type KeyMap struct {
	Up, Down, Select, Back, Top, Bottom, FilterRepo, FilterAuthor, FilterLabel, ToggleUnread, Sync, Claude, Next, Prev, Quit key.Binding
}
...
var Keys = KeyMap{
	...
	FilterRepo:   key.NewBinding(key.WithKeys("f"), key.WithHelp("f", "filter repo")),
	FilterAuthor: key.NewBinding(key.WithKeys("a"), key.WithHelp("a", "filter author")),
	FilterLabel:  key.NewBinding(key.WithKeys("d"), key.WithHelp("d", "filter labels")),
